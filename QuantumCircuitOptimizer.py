...
class QuantumCircuitOptimizer:
    def __init__(self,circuit,parameters,parameter_values,cost_function,learning_rate,backends,epochs=100):
        ...
        for backend in self.backends:
            self.backend_dic[backend.name()] = 0
            self.loss_dic[backend.name()] = 0

        print(self.backends_dic)
        print(self.loss_dic)
    ...
    def get_cost_device(self, device):
        circuit = self.circuit.assign_parameters(self.parameter_values)
        circ = qiskit.transpile(circuit,device)
        result = sim.run(circ).result().get_counts()
        result = self.complete_results(result)
        keys = list(result.keys())
        keys.sort()
    ...
    def train(self,epochs=250,learning_rate=0.,validate=True):
        ...
        while True:
            ...
            print(f"Ready: {ready}")
            for obj in ready:
                diff_index,grad,p_correct, machine = ray.get(obj)
                self.backend_dic[machine.name()] += 1
                # ========================================================================================
                ...
                if curr_epoch != old_epoch:
                    ...
                    if (curr_epoch %) 10 == 0:
                        path = str(pathlib.Path(__file__).parent.resolve()) +"/results"
                        ...
                        with open(path+'/ob_access'+str(curr_epoch)+'.txt','w') as f:
                            f.write(str(self.backend_dic))
            ...
            if curr_epoch == epochs:
                for backend in self.backends:
                    loss = get_cost_device(backend)
                    loss_dic[backend.name()] = loss

                    with open(path+'/ob_loss.txt','w') as f:
                        f.write(str(self.loss_dic))
                break
